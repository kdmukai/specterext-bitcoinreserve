...
@click.option(
    "--host",
    default="127.0.0.1",
    help="if you specify --host 0.0.0.0 then Bitcoinreserve will be available in your local LAN.",
)
@click.option(
    "--ssl/--no-ssl",
    is_flag=True,
...
def start(ctx, host, ssl, debug, filelog, config):
    if config == None:
        config = "kdmukai.specterext.bitcoinreserve.config.AppProductionConfig"
